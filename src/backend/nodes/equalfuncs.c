...
_equalReindexStmt(const ReindexStmt *a, const ReindexStmt *b)
{
	...
	COMPARE_SCALAR_FIELD(concurrentlyPhase);
	COMPARE_NODE_FIELD(newIndexInfo);
	COMPARE_NODE_FIELD(oldIndexInfo);

	return true;
}

static bool
_equalReindexIndexInfo(const ReindexIndexInfo *a, const ReindexIndexInfo *b)
{
	COMPARE_SCALAR_FIELD(indexId);
	COMPARE_SCALAR_FIELD(tableId);
	COMPARE_SCALAR_FIELD(amId);
	COMPARE_SCALAR_FIELD(safe);
	COMPARE_STRING_FIELD(ccNewName);
	COMPARE_STRING_FIELD(ccOldName);
...
equal(const void *a, const void *b)
{
	...
	switch (nodeTag(a))
	{
		...
		case T_ReindexIndexInfo:
			retval = _equalReindexIndexInfo(a, b);
			break;
